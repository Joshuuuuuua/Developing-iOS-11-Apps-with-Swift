...
///
/// made for testing viewController life cycle
///
class VCLLoggingViewController: UIViewController {
    ...
    private static func logPrefix(for loggingName: String) -> String {
        if VCLLoggingViewController.logGlobals.lastLogTime.timeIntervalSinceNow < -VCLLoggingViewController.logGlobals.indentationInterval {
            VCLLoggingViewController.logGlobals.prefix += VCLLoggingViewController.logGlobals.indentationString
            print("")
        }
        VCLLoggingViewController.logGlobals.lastLogTime = Date()
        return VCLLoggingViewController.logGlobals.prefix + loggingName
    }
    
    private static func bumpInstanceCount(for loggingName: String) -> Int {
        VCLLoggingViewController.logGlobals.instanceCounts[loggingName] = (VCLLoggingViewController.logGlobals.instanceCounts[loggingName] ?? 0) + 1
        return VCLLoggingViewController.logGlobals.instanceCounts[loggingName]!
    }
    ...
    override func viewWillTransition(to size: CGSize, with coordinator: UIViewControllerTransitionCoordinator) {
        ...
        coordinator.animate(
            alongsideTransition: { (context: UIViewControllerTransitionCoordinatorContext!) -> Void in
                self.logVCL("begin animate(alongsideTransition:comletion:)")
        },
            completion: {context -> Void in
                self.logVCL("end animate(alongsideTransition:comletion:)")
        })
    }
